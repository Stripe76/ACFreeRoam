...
namespace ACDBackend
{
    ...
    {
        public void saveACD(string path, ACDFiles entries)
        {
            //Get size of entries array.

            using (MemoryStream stream = new MemoryStream())
            {
                foreach (ACDFile entry in entries)
                {
                    writeString(entry.fileName, stream); //Write the name of the file.
        ...
        public int getEntriesSize(ACDFiles entries)
        {
            int returnSize = 0;

            foreach (ACDFile entry in entries)
            {
                returnSize += entry.fileName.Length + entry.fileData.Length;
            }
