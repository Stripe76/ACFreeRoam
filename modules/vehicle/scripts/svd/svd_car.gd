class_name SVD_BODY extends Vehicle
...
@export_group("Differential","DIFF_")
...
func controls(d_scale):
	...
	var steer_desired: float = inputs.steering
	var desired_accelerate: float = 0.0
	var desired_decelerate: float = 0.0
	var desired_handbrake: float = 0.0
	var desired_clutching: float = 0.0
...
func _integrate_forces(state: PhysicsDirectBodyState3D):
	...
	var drive_median: float = 0.0
	var central_median: float = 0.0
	var central_travel_median: float = 0.0
	var fastest_wheel: float = 0.0
